...
def separate(desc='Bastien_Cervix_Uterus_Data_No_Applicator',path_base=r'K:\Morfeus\BMAnderson\CNN\Data\Data_Bastien'):
    ...
    for series_descr in patient_info.keys():
        patient_dict = patient_info[series_descr]
        keys = list(patient_dict.keys())
        if 'path' in keys:
            del keys[keys.index('path')]
        all_images = os.path.join(path,series_descr)
        train_path = os.path.join(path,series_descr,'Train')
        test_path = os.path.join(path, series_descr,'Test')
        validation_path = os.path.join(path,series_descr,'Validation')
